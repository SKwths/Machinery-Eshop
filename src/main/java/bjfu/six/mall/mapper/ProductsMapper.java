...
public interface ProductsMapper {
    ...
    @Select("select * from action_products where product_id = #{productId} limit #{offset}, #{limit}")
    Products[] selectProductsByClassID(int productId, int offset, int limit);
    ...
    @Select("select * from action_products where name like '%${keyword}%' and product_id = ${productId} limit #{offset}, #{limit}")
    Products[] selectProductsByParamIDAndName(int productId, String keyword, int offset, int limit);
    ...
    @Update("update action_products set name=#{name},product_id=#{productId},icon_url=#{icon_url},sub_images=#{sub_images},detail=#{detail},spec_param=#{spec_param},price=#{price},stock=#{stock} where id=#{id}")
    int updateProductsById(Integer id, String name, Integer productId, String icon_url, String sub_images, String detail, String spec_param, Integer price, Integer stock);
    ...
    @Select("select count(*) from action_products where name like '%${keyword}%' and product_id = ${productId}")
    int selectProductsByClassIDAndNameCount(int productId, String keyword);

    @Select("select count(*) from action_products where product_id = #{productId}")
    int selectProductsByClassIDCount(int productId);
